...
module.exports = {
  initConnection: function () {
    ...
    try {
      self.log(
        "info",
        `Opening connection to ${self.config.host}:${self.config.port}`,
      );
  ...
  getInformation: function (scope) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      if (self.step === 0)
        self.sendCommand(
          hexproto.buildHexCommand(hexproto.MSGTYPE.CMD, "01D6"),
        ); // power
      if (self.step === 1)
        self.sendCommand(
          hexproto.buildHexCommand(
            hexproto.MSGTYPE.GET_PARAM,
            hexproto.OPPAGE_INPUT + hexproto.OPCODE_INPUT,
          ),
        ); // input
      if (self.step === 2)
        self.sendCommand(
          hexproto.buildHexCommand(
            hexproto.MSGTYPE.GET_PARAM,
            hexproto.OPPAGE_VOLUME + hexproto.OPCODE_VOLUME,
          ),
        ); // volume
  ...
  processData: function (data) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      if (self.config.verbose)
        self.log("info", "Processing HEX data: " + data.toString("hex"));
      let response = hexproto.parseData(data);
      if (self.config.verbose)
        self.log("info", "Processed response: " + JSON.stringify(response));
      ...
      if (response.msgType === hexproto.MSGTYPE.CMD_REPLY) {
        ...
        if (self.config.verbose)
          self.log("debug", "CMD Reply payload is: " + payload);
        if (
          payload === "30323030443630303030303430303034" ||
          payload === "303043323033443630303034"
        ) {
          self.data.power = "off";
        } else if (
          payload === "30323030443630303030303430303031" ||
          payload === "303043323033443630303031"
        ) {
          self.data.power = "on";
        }
      } else if (
        response.msgType === hexproto.MSGTYPE.GET_PARAM_REPLY ||
        response.msgType === hexproto.MSGTYPE.SET_PARAM_REPLY
      ) {
        // get parameter reply
        ...
        if (self.config.verbose)
          self.log(
            "debug",
            "PARAM Reply payload is: " + JSON.stringify(replyData),
          );

        if (replyData.success !== true) {
          self.log(
            "warn",
            `Get/Set Param failed: OP Page ${replyData.opPage}, OP Code ${replyData.opCode}, Value ${replyData.value}`,
          );
        } else {
          if (
            replyData.opPage == hexproto.OPPAGE_INPUT &&
            replyData.opCode == hexproto.OPCODE_INPUT
          ) {
            // input
            self.data.input = hexproto.getMappedInput(replyData.value);
          } else if (
            replyData.opPage == hexproto.OPPAGE_VOLUME &&
            replyData.opCode == hexproto.OPCODE_VOLUME
          ) {
            // volume
            self.data.volume = replyData.value;
          } else if (
            replyData.opPage == hexproto.OPPAGE_AUDIOMUTE &&
            replyData.opCode == hexproto.OPCODE_AUDIOMUTE
          ) {
            // audio mute
  ...
  setPower: function (inpower) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      let cmd = hexproto.buildHexCommand(
        hexproto.MSGTYPE.CMD,
        "C203D6000" + (power === "on" ? "1" : "4"),
      );
  ...
  setInput: function (input) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      if (cmd !== "") {
        cmd = hexproto.buildHexCommand(
          hexproto.MSGTYPE.SET_PARAM,
          hexproto.OPPAGE_INPUT + hexproto.OPCODE_INPUT + cmd,
        );
        self.sendCommand(cmd);
      }
    }
  ...
  setVolume: function (volume) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      let cmd = hexproto.buildHexCommand(
        hexproto.MSGTYPE.SET_PARAM,
        hexproto.OPPAGE_VOLUME +
          hexproto.OPCODE_VOLUME +
          volume.toString(16).padStart(4, "0"),
      );
      self.sendCommand(cmd);
    }
  },
  ...
  setAudioMute: function (mute) {
    ...
    if (self.config.protocol == "ascii") {
      ...
      let cmd = hexproto.buildHexCommand(
        hexproto.MSGTYPE.SET_PARAM,
        hexproto.OPPAGE_AUDIOMUTE +
          hexproto.OPCODE_AUDIOMUTE +
          (self.data.audiomute ? 1 : 0).toString(16).padStart(4, "0"),
      );
      self.sendCommand(cmd);
    }
  },
